...
import DarkWorldMap from "@/components/ui/dark-world-map";

interface HomeHeroAProps {
  onExploreClick: () => void;
}

const HomeHeroA: React.FC<HomeHeroAProps> = ({ onExploreClick }) => {
  ...
  // Melbourne coordinates for world map connections (adjusted for proper positioning)
  ...
  return (
    <section className="relative h-[90vh] sm:h-[85vh] md:h-screen pt-32 md:pt-16 overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col items-center justify-center pt-8 pb-8 h-full text-center px-4 animate-fade-in-up">
        <Badge
          ref={badgeRef}
          variant="outline"
        ...
        <h1 
          ref={titleRef}
          className="text-4xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-semibold mb-2 leading-tight px-2"
          style={{
        ...
        <div ref={buttonsRef} className="flex flex-col sm:flex-row gap-3 sm:gap-4 mt-4 w-full sm:w-auto px-4 sm:px-0">
          <button
